<template>
  <div class="uni-scroll-container" ref="scrollContainer">
    ...
    <div class="floating-geo-btn position-fixed end-0 z-3">
      ...
      >
        ...
        <span class="d-none d-sm-inline">
          {{ userLat ? "Updated Nearby" : "Find Near Me" }}</span
        >
    ...
    <!-- SEARCH & FILTER BAR -->
    <div
      class="position-fixed top-0 start-0 w-100 z-search-filter filter-bar-nav-margin"
    >
    ...
    <Transition name="overlay-fade">
      ...
      >
        ...
        <h3 class="fw-bold text-gold mb-2 fs-4 fs-md-3">
          Calculating Proximity...
        </h3>
        <p class="text-light opacity-75 fs-6 fs-md-5">
          Finding campuses closest to your live location
        </p>
      </div>
    </Transition>
    ...
    <Transition name="card-fade">
      <div
        ...
        class="fullscreen-card d-flex align-items-center justify-content-center text-white bg-navy p-3"
      >
        <div class="text-center pt-5">
          ...
          <button
            @click="resetFilters"
            class="btn btn-gold rounded-pill px-4 fw-bold"
          >
            Reset Filters
          </button>
        </div>
      ...
      >
        ...
        >
          ...
          <div
            ...
            :style="{
              background: `linear-gradient(180deg, rgba(${uni.themeColor}, 0.7) 0%, rgba(${uni.themeColor}, 0.95) 100%)`,
            }"
          ></div>
        </div>
        ...
        <div class="card-content text-center z-1 w-100 max-w-lg">
          ...
          <div
            class="d-flex justify-content-center gap-2 mb-2 mb-md-3 flex-wrap"
          >
            <span
              class="badge bg-gold text-dark fw-bold px-3 py-2 rounded-pill text-uppercase fs-8 fs-md-7"
            >
              {{ uni.type }}
            </span>
            <span
              v-if="uni.distanceKm"
              class="badge bg-light text-dark fw-bold px-3 py-2 rounded-pill fs-8 fs-md-7 shadow-sm distance-pop-badge"
            >
              <i class="bi bi-pin-map-fill text-danger me-1"></i>
              {{ uni.distanceKm }} km away
            </span>
          </div>
          ...
          <div class="row g-3 my-2 text-start">
            ...
            <div class="col-12 col-md-6">
              <div class="p-3 rounded bg-navy-card h-100">
                ...
                <p class="small text-light mb-0">
                  {{
                    uni.facultiesRaw ||
                    "Consult official prospectus for full list."
                  }}
                </p>
              </div>
            </div>
            ...
            <div class="col-12 col-md-6">
              <div
                class="p-3 rounded bg-navy-card h-100 d-flex flex-column justify-content-between"
              >
          ...
          <!-- ACTION BUTTONS -->
          <div
            class="d-flex flex-column flex-sm-row justify-content-center align-items-stretch align-items-sm-center gap-2 gap-sm-3 max-btn-width mx-auto mt-3"
          >
            <button
            ...
            <a
              ...
              :class="{
                disabled: !uni.applicationUrl || uni.applicationUrl === '#',
              }"
            >
        ...
        <!-- SCROLL INDICATOR -->
        <div
          class="scroll-hint position-absolute bottom-0 start-50 translate-middle-x mb-2 mb-md-4 text-center text-light opacity-75"
        >
    ...
    >
      <div
        class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable p-2 p-sm-0"
      >
        <div class="modal-content bg-dark text-white border-gold shadow-lg">
          <div class="modal-header border-bottom border-secondary">
            <div>
              <h5 class="modal-title fw-bold mb-0 fs-6 fs-md-4">
                {{ selectedUni.name }}
              </h5>
              <small class="text-gold fs-7">
                {{ selectedUni.location }} • {{ selectedUni.province }}
                <span v-if="selectedUni.distanceKm">
                  • {{ selectedUni.distanceKm }} km away</span
                >
              </small>
            </div>
            <button
          ...
          <div class="modal-body py-3 py-md-4">
            ...
            <p class="text-light fs-7 fs-md-6 mb-4">
            ...
            <div class="row g-2 g-md-3 mb-4">
              <div class="col-12 col-md-6">
                <div class="p-3 rounded bg-navy border border-secondary h-100">
                  <h6 class="fw-bold text-gold mb-1 fs-7 fs-md-6">
                    <i class="bi bi-book me-2"></i>Key Faculties
                  </h6>
                  <ul class="mb-0 ps-3 small text-light">
                    <li v-for="(faculty, i) in selectedUni.faculties" :key="i">
                      {{ faculty }}
                    </li>
                  </ul>
                </div>
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded bg-navy border border-secondary h-100">
                  <h6 class="fw-bold text-gold mb-1 fs-7 fs-md-6">
                    <i class="bi bi-cash-stack me-2"></i>Application Fee
                  </h6>
                  <p class="mb-0 fs-6 fw-semibold text-white">
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded bg-navy border border-secondary h-100">
                  ...
                  <p class="mb-0 small text-light">
                    <strong>Opens:</strong>
                    {{ formatDate(selectedUni.opening_date) }}<br />
                    <strong>Closes:</strong>
                    {{ formatDate(selectedUni.closing_date) }}
                  </p>
                </div>
              </div>
              <div class="col-12 col-md-6">
                <div class="p-3 rounded bg-navy border border-secondary h-100">
                  <h6 class="fw-bold text-gold mb-1 fs-7 fs-md-6">
                    <i class="bi bi-patch-check me-2"></i>Status
                  </h6>
            ...
            <p class="small text-light mb-0 fs-7">
              {{ selectedUni.requirements }}
            </p>
          </div>

          <div
            class="modal-footer border-top border-secondary flex-column flex-sm-row justify-content-between gap-2"
          >
            <button
              type="button"
              class="btn btn-outline-light rounded-pill px-4 w-100 w-sm-auto"
              @click="closeDetails"
            >
              Close
            </button>
            <button
              type="button"
              class="btn btn-outline-warning text-gold border-gold rounded-pill px-4 w-100 w-sm-auto"
              @click="openReminderModal(selectedUni)"
            >
              <i class="bi bi-bell me-1"></i> Remind Me
            </button>
            <a
              :href="selectedUni.applicationUrl"
              target="_blank"
              rel="noopener noreferrer"
              class="btn btn-gold text-dark fw-bold rounded-pill px-4 w-100 w-sm-auto text-center"
            >
              Apply Portal
            </a>
          </div>
    ...
    >
      <div class="modal-dialog modal-dialog-centered">
        <div
          class="modal-content bg-dark text-white border-gold shadow-lg text-center p-3"
        >
          <div
            class="modal-header border-bottom border-secondary justify-content-center"
          >
            <h5 class="modal-title fw-bold text-gold">Premium Feature Only</h5>
          </div>
          <div class="modal-body py-4">
            <p class="text-light mb-3">
              This is for premium users only. Unlock instant reminders,
              fast-track applications, and exclusive concierge features!
            </p>
          </div>
          <div
            class="modal-footer border-top border-secondary justify-content-center gap-2"
          >
            <button
              type="button"
            ...
            <button
              type="button"
              class="btn btn-gold text-dark fw-bold rounded-pill px-4"
...
export default {
  ...
  computed: {
    filteredInstitutions() {
      const query = (this.searchFilter?.searchQuery || this.searchQuery || "")
        .trim()
        .toLowerCase();
      const province = (
        this.searchFilter?.selectedProvince ||
  ...
  methods: {
    ...
    getStatusBadgeClass(institution) {
      ...
      if (status === "Opening Soon")
        return "badge bg-warning text-dark px-3 py-2 fs-7";
    ...
    getUserLocation() {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          this.isLocating = false;
          alert(
            "Unable to access location. Please check browser GPS permissions.",
          );
          console.error(error);
        },
    ...
    handleImageError(event, index) {
      ...
      event.target.src =
        this.fallbackPhotos[index % this.fallbackPhotos.length];
    },
      ...
      try {
        ...
        if (result.success) {
          ...
          this.institutions = result.data.map((item, index) => {
            ...
            if (!lat || !lng) {
              const matchedKey = Object.keys(coordinatesMap).find((key) =>
            ...
            return {
              ...
              requirements:
                item.requirements ||
                "National Senior Certificate (NSC) or equivalent with minimum required APS points for selected program.",
              ...
              description:
                item.description ||
                `${item.name} is a higher education institution located in ${item.province}, South Africa.`,
            };
          });
