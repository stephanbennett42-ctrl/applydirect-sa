...
async function ensureMySQL() {
  ...
  if (process.platform === 'win32') {
    ...
    if (fs.existsSync(mysqld)) {
      const child = spawn(mysqld, [
        '--port=3307', '--basedir=C:\\xampp\\mysql', '--datadir=C:\\xampp\\mysql\\data'
      ], { stdio: 'ignore', detached: true, windowsHide: true })
      child.unref()
      log('XAMPP MariaDB starting on port 3307 (in background).')
    } else {
      log('ERROR: XAMPP not found at C:\\xampp. Please start MySQL/MariaDB on port 3307 yourself.')
      return false
    }
  } else {
    try {
    ...
    } catch { /* error is handled by the port check below */ }
    log('Attempted to start MySQL/MariaDB. If it is not running, start it yourself on port 3307 before re-running this script.')
  }
  ...
  log('ERROR: MySQL did not start on time. Verify MySQL is configured to listen on port 3307, then re-run. Windows users: install/start XAMPP.')
  return false
}
...
function runNpm(cwdRel, npmArgs) {
  ...
  return spawnSync(cmd, npmArgs, { cwd, stdio: 'inherit', shell: process.platform === 'win32' })
}
...
function startDaemon(app) {
  ...
  const logPath = path.join(LOG_DIR, app.name.replace(/\s+/g, '-').toLowerCase() + '.log')
  const outFd = fs.openSync(logPath, 'a')

  const script = app.type === 'backend'
    ? path.join(cwd, 'server.js')
    : path.join(cwd, 'node_modules', 'vite', 'bin', 'vite.js')

  if (!fs.existsSync(script)) {
    log(`ERROR: Cannot find ${script}. Run "npm install" in that folder, or run the launcher once more.`)
  ...
  child.unref()
  try { fs.closeSync(outFd) } catch { /* ignore */ }
  log(`Started ${app.name} (pid ${child.pid}) -> log: ${path.relative(ROOT, logPath)}`)
