...
router.post('/:id/pay', requireAuth, async (req, res) => {
  try {
    ...
    const [orderRows] = await db.query('SELECT * FROM orders WHERE id = ? AND user_id = ?', [orderId, req.user.id])
    ...
    await db.query('UPDATE orders SET status = ?, payment_method = ? WHERE id = ?', ['paid', method, orderId])
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const [payRows] = await db.query('SELECT * FROM payments WHERE order_id = ?', [o.id])
